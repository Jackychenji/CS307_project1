...
//            System.out.println(preparedStatement.toString());
    ...
    public String select_ReCode() {
        getConnection();
        StringBuilder sb = new StringBuilder();
        String sql = "select retrieval_city_code from item_transportation_information1 group by retrieval_city_code";
        try {
            Statement statement = con.createStatement();
            resultSet = statement.executeQuery(sql);
            while (resultSet.next()) {
    ...
    public String Select_ReCode_count() {
        getConnection();
        StringBuilder sb = new StringBuilder();
        String sql = "select retrieval_city_code ,count(*) as numbers from item_transportation_information1 group by retrieval_city_code;";
        try {
            Statement statement = con.createStatement();
            resultSet = statement.executeQuery(sql);
            while (resultSet.next()) {
                sb.append(resultSet.getString("retrieval_city_code") + "\t");
                sb.append(resultSet.getString("numbers"));
    ...
    public String Select_by_ReCode() {
        getConnection();
        StringBuilder sb = new StringBuilder();
        String sql = "select item_name from item_transportation_information1 where retrieval_city_code='1084';";
        try {
            Statement statement = con.createStatement();
            resultSet = statement.executeQuery(sql);
            while (resultSet.next()) {
